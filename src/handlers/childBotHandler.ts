...
export const setupChildBot = (bot: Telegraf<Context>, botData: IBot) => {
  ...
  const getListSettingsMenu = (b: IBot) => {
    ...
    return Markup.inlineKeyboard([
      ...
      [Markup.button.callback('➖', 'dec_cols'), Markup.button.callback(`🔢 الأعمدة: ${b.columnsCount}`, 'none'), Markup.button.callback('➕', 'inc_cols')],
      [Markup.button.callback(`⚖️ الترتيب: ${sortLabels[b.sortType || 'members_desc']}`, 'cycle_sort')],
  ...
  const getReceptionMenu = (b: IBot) => Markup.inlineKeyboard([
    ...
    [Markup.button.callback('➖', 'dec_min'), Markup.button.callback(`👥 الحد الأدنى: ${b.minMembers}`, 'none'), Markup.button.callback('➕', 'inc_min')],
  ...
  const getWatchMenu = (b: IBot) => Markup.inlineKeyboard([
    ...
    [Markup.button.callback('➖', 'dec_bump'), Markup.button.callback(`⚙️ حد الرفع: ${b.bumpThreshold || 5}`, 'none'), Markup.button.callback('➕', 'inc_bump')],
    [Markup.button.callback('🔙 عودة', 'back_main')]
  ]);
  ...
  bot.on('callback_query', async (ctx) => {
    ...
    // --- أزرار الزيادة والنقصان التفاعلية ---
    else if (data === 'inc_cols') { b.columnsCount = Math.min(5, b.columnsCount + 1); await b.save(); ctx.editMessageReplyMarkup(getListSettingsMenu(b).reply_markup); }
    else if (data === 'dec_cols') { b.columnsCount = Math.max(1, b.columnsCount - 1); await b.save(); ctx.editMessageReplyMarkup(getListSettingsMenu(b).reply_markup); }
    else if (data === 'inc_min') { b.minMembers += 10; await b.save(); ctx.editMessageReplyMarkup(getReceptionMenu(b).reply_markup); }
    else if (data === 'dec_min') { b.minMembers = Math.max(0, b.minMembers - 10); await b.save(); ctx.editMessageReplyMarkup(getReceptionMenu(b).reply_markup); }
    else if (data === 'inc_bump') { b.bumpThreshold = (b.bumpThreshold || 5) + 1; await b.save(); ctx.editMessageReplyMarkup(getWatchMenu(b).reply_markup); }
    else if (data === 'dec_bump') { b.bumpThreshold = Math.max(1, (b.bumpThreshold || 5) - 1); await b.save(); ctx.editMessageReplyMarkup(getWatchMenu(b).reply_markup); }
    ...
    // --- تحديث القنوات ---
      ...
      if (channelsWithList.length > 0) return ctx.answerCbQuery('⚠️ القائمة منشورة بالفعل.', { show_alert: true });
      ...
      userStates.set(ctx.from!.id, { action: 'edit_max_name' });
      ctx.reply('📏 أرسل أقصى طول مسموح به لاسم القناة:', Markup.inlineKeyboard([Markup.button.callback('❌ إلغاء', 'menu_reception')]));
    }
  });
  ...
  bot.on('message', async (ctx, next) => {
    ...
    if (state?.action === 'edit_max_name') {
      const val = parseInt(msg.text);
      if (!isNaN(val)) { (b as any).maxNameLength = val; await b.save(); userStates.delete(userId); return ctx.reply('✅ تم تحديث أقصى طول.', getMainMenu()); }
    }
    return next();
  });
};
...
async function handleChannelAdd(bot: Telegraf<Context>, b: any, ctx: Context, channelIdentifier: string | number) {
  try {
    ...
    await Channel.create({
      botId: b._id, ownerId: ctx.from!.id, channelId: chat.id, title: (chat as any).title, inviteLink: (chat as any).invite_link || `https://t.me/${(chat as any).username}`, memberCount: memberCount, isApproved: true
    });
    ctx.reply(`✅ تم قبول القناة: ${(chat as any).title}`);
    if (b.adminGroupId) await bot.telegram.sendMessage(b.adminGroupId, `➕ قناة جديدة: ${(chat as any).title}`);
